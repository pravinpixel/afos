        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)->addIndexColumn()
                ->addColumn('status', function($row){
                    if( $row->status == 1){
                        if (check_user_access('product-category', 'editable')) {
                ...
                ->addColumn('action', function($row){
                    ...
                    if (check_user_access('product-category', 'delete')) {
                        $del_btn = '<a href="javascript:void(0);" class="action-icon" onclick="return delete_category('.$row->id.')"> <i class="mdi mdi-delete"></i></a>';
                    }
